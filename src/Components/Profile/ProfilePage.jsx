...
export default function ProfilePage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="relative flex items-center justify-center">
          ...
          <Loader2 className="h-6 w-6 animate-spin text-indigo-400 absolute" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto px-4 py-8 text-zinc-100">
      ...
      {user?.isFraud && (
        <div className="flex items-center gap-4 p-5 rounded-2xl border border-red-500/20 bg-red-950/20 text-red-400 shadow-xl shadow-red-950/10 animate-pulse backdrop-blur-md">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5 font-medium">Warning: Your account has been flagged for fraudulent activity by the central governance panel.</p>
          </div>
        </div>
      )}

      <div className="relative rounded-[32px] bg-zinc-950/40 backdrop-blur-xl shadow-2xl border border-indigo-500/10 overflow-hidden transition-all duration-300">
        <div className="h-40 sm:h-48 bg-gradient-to-r from-zinc-950 via-indigo-950/60 to-zinc-950 relative border-b border-zinc-800/50">
        ...
        <div className="p-6 sm:p-8 pt-0 flex flex-col sm:flex-row items-center sm:items-end justify-between relative -mt-20 sm:-mt-24 sm:space-x-6 space-y-6 sm:space-y-0 text-center sm:text-left">
          ...
          <div className="relative inline-block shrink-0 group">
            <div className="absolute inset-0 bg-indigo-500/20 rounded-3xl blur-xl group-hover:blur-2xl transition-all opacity-70" />
            <Image 
              width={200} 
              height={200}
              src={user?.image || defaultAvatar}
              alt="Profile Avatar"
              className="h-32 w-32 sm:h-40 sm:w-40 rounded-3xl object-cover ring-4 ring-zinc-900/80 shadow-2xl bg-zinc-900 relative z-10 border border-zinc-800"
            />
          </div>
          ...
          <div className="flex-1 min-w-0 z-10">
            ...
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 mt-3">
              <span className="inline-flex items-center px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest bg-indigo-600/20 border border-indigo-500/30 text-indigo-400">
                {user?.role || "user"} Mode
              </span>
              <span className="text-xs text-zinc-500 font-bold flex items-center gap-1.5 bg-zinc-900/60 px-2.5 py-1 rounded-xl border border-zinc-800/40">
                <Cpu className="h-3.5 w-3.5 text-zinc-600" /> ID: {user?.id?.slice(0, 12) || "N/A"}...
              </span>
            </div>
          </div>
          ...
          <button
            onClick={() => setIsModalOpen(true)}
            className="inline-flex items-center space-x-2 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs rounded-2xl shadow-lg shadow-indigo-600/10 transition-all duration-300 w-full sm:w-auto justify-center uppercase tracking-widest active:scale-[0.98] border border-indigo-500/20"
          >
            <Edit2 className="h-3.5 w-3.5" />
      ...
      <div className="bg-zinc-950/40 backdrop-blur-xl rounded-[32px] shadow-2xl border border-indigo-500/10 p-6 sm:p-8 space-y-6">
        <h2 className="text-xs font-black text-indigo-400 flex items-center gap-2 border-b border-zinc-800/60 pb-4 uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block pl-1">Legal Name</label>
            <div className="relative flex items-center">
              <span className="absolute left-4 flex items-center pointer-events-none z-10">
                <User className="h-4 w-4 text-zinc-600" />
              </span>
              <div className="w-full pl-12 pr-4 py-4 text-xs sm:text-sm font-semibold rounded-2xl border border-zinc-800 bg-zinc-900/30 text-zinc-300 select-all tracking-wide">
                {user?.name || "Not Specified"}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block pl-1">Communications Relay</label>
            <div className="relative flex items-center">
              <span className="absolute left-4 flex items-center pointer-events-none z-10">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block pl-1">Scope Privilege</label>
            <div className="relative flex items-center">
              <span className="absolute left-4 flex items-center pointer-events-none z-10">
                <ShieldAlert className="h-4 w-4 text-zinc-600" />
              </span>
              <div className="w-full pl-12 pr-4 py-4 text-xs sm:text-sm font-semibold rounded-2xl border border-zinc-800 bg-zinc-900/30 text-zinc-400 capitalize tracking-wide">
                {user?.role || "user"}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block pl-1">Verification Status</label>
            <div className="relative flex items-center">
              <span className="absolute left-4 flex items-center pointer-events-none z-10">
                <Calendar className="h-4 w-4 text-zinc-600" />
              </span>
              <div className={`w-full pl-12 pr-4 py-4 text-xs sm:text-sm font-black rounded-2xl border border-zinc-800 bg-zinc-900/30 tracking-wide ${
