  ...
  return (
    <div className="space-y-6 text-slate-800 dark:text-zinc-200">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-5 rounded-3xl flex items-center justify-between shadow-xs">
          <div>
          ...
          <div className="p-3 bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400 rounded-2xl"><Users className="h-5 w-5" /></div>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 p-4 border border-slate-200 dark:border-zinc-800 rounded-3xl flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-black border border-slate-200 dark:border-zinc-800 pl-10 pr-4 py-2.5 rounded-xl text-xs font-bold focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-zinc-900 transition-all text-slate-800 dark:text-zinc-200"
          />
        </div>
      ...
      <div className="bg-slate-100/60 dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800/60 rounded-2xl p-3 px-4 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[11px] text-slate-500 dark:text-zinc-400 font-bold">
        <span className="flex items-center gap-1 text-slate-400 dark:text-zinc-500"><Info className="h-3.5 w-3.5" /> Command Console Matrix:</span>
            ...
            <table className="w-full border-collapse text-left min-w-[900px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/40 text-xs font-bold text-slate-700 dark:text-zinc-300">
                {currentItems.map((user) => (
                  <tr key={user._id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-950/20 transition-colors">
                    <td className="p-4 pl-6">
                      <div className="relative w-9 h-9">
                        <Image
                          ...
                          width={36}
                          height={36}
                          className="w-9 h-9 object-cover rounded-full ring-2 ring-slate-200 dark:ring-zinc-800"
                        />
                      </div>
                    </td>
                    <td className="p-4 font-black text-xs text-slate-900 dark:text-white">{user.name}</td>
                    <td className="p-4 font-mono text-slate-400 dark:text-zinc-500 text-[11px]">{user.email}</td>
      ...
      <AnimatePresence>
        {modalContext.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="w-full max-w-md bg-white dark:bg-zinc-950 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-2xl p-6 space-y-6 relative z-10 text-left">
              ...
              <div className="flex items-center justify-end space-x-3 pt-2">
                <button onClick={closeActionModal} className="px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:text-zinc-300 transition-colors cursor-pointer">Abort Command</button>
                <button
